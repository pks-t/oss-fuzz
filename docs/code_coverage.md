...
You might want to use some of the options of [llvm-cov tool] such as
`-ignore-filename-regex`. You can pass those to the helper script via
`--extra-args` flag:

```bash
python infra/helper.py profile --extra-args='-ignore-filename-regex=.*code/to/be/ignored/.*' $project_name
```
